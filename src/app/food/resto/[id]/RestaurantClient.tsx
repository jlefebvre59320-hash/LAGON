...
import { connexionUrl, emailHref, safeExternalUrl, socialUrl } from "@/lib/urls";
...
export default function RestoPage({ initialRestaurant = null }: { initialRestaurant?: Restaurant | null }) {
  return (
    <FavoritesProvider kind="restaurant">
      <Resto initialRestaurant={initialRestaurant} />
    </FavoritesProvider>
  );
}

function Resto({ initialRestaurant }: { initialRestaurant: Restaurant | null }) {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();
  const { userId } = useSession();
  const [r, setR] = useState<Restaurant | null>(initialRestaurant);
  ...
  async function rate(n: number) {
    ...
    if (!session.session) { router.push(connexionUrl(`/food/resto/${id}`)); return; }
    setRateBusy(true);
    const prev = myRating;
    setMyRating(n);
  ...
  const hasExternalLinks = instagram || facebook || snapchat || tiktok || website || email;
      ...
      <main className="container" style={{ paddingTop: 16, paddingBottom: 110, maxWidth: 740, flex: 1 }}>
        ...
        {hasExternalLinks && (
          <div style={{ display: "flex", gap: 14, marginTop: 14, flexWrap: "wrap", fontSize: 13 }}>
            ...
            {facebook && (
              <a href={facebook} target="_blank" rel="noopener noreferrer" style={{ color: "var(--gold-deep)", fontWeight: 600 }}>
                Facebook ↗
              </a>
            )}
            {snapchat && (
            ...
            {website && (
              <a href={website} target="_blank" rel="noopener noreferrer" style={{ color: "var(--gold-deep)", fontWeight: 600 }}>
                Site web ↗
              </a>
            )}
            {email && (
              <a href={email} style={{ color: "var(--gold-deep)", fontWeight: 600 }}>
                Email ↗
              </a>
            )}
          </div>
