...
| Accès et conditions | **Pas d'API officielle ; aucun lien vers des conditions d'utilisation dans la page de ligue** (V1, page lue le 2026-09-06 depuis le poste). Depuis 2025 la page HTML est vide (18 Ko) et le navigateur charge les données par `GET /getLeagueData/{league}/{season}` (V1, lu dans `js/league.min.js`) ; le serveur répond 404 sans en-têtes XMLHttpRequest et renvoie du JSON gzip (~90 Ko compressés, ~530 Ko décompressés pour EPL 2014) structuré par club (`teams[id].history[]` : `h_a, date, scored, missed, xG, xGA, npxG, npxGA, xpts, ppda, deep`). Réutilisation : zone grise, un cran au-delà de la lecture d'une page. Décision : extraction lente (≥ 6 s), cache complet du brut, usage personnel, pas de redistribution, arrêt définitif au premier 403 ou 429. |
